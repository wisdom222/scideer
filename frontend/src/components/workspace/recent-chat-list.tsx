...
export function RecentChatList() {
  ...
  const handleShare = useCallback(
    async (thread: AgentThread) => {
      // Use deployed server URL for sharing so others can access
      const SHARE_BASE_URL = "http://43.156.100.148:2026";
      const isLocalhost =
        window.location.hostname === "localhost" ||
        window.location.hostname === "127.0.0.1";
      // On localhost: use deployed URL; On production: use current origin
      const baseUrl = isLocalhost ? SHARE_BASE_URL : window.location.origin;
