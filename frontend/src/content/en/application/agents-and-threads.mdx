...
description: Application supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
Application supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
<Callout type="info">
  ...
  to the LangGraph Server deployment used by Application.
</Callout>
...
```
backend/.scideer/threads/{thread_id}/user-data/
```

This directory is mounted into the sandbox as `/mnt/user-data/` for the agent to read and write.

In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `DEER_FLOW_ROOT` to the absolute path of your scideer repository root to ensure the correct host path is used.
