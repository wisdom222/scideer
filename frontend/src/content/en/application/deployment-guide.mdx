...
description: "This guide covers all supported deployment methods for Application: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for Application: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run Libra. All services run as native processes on your machine.
...
| ----------- | ---- | ------------------------ |
| LangGraph   | 2024 | Harness runtime |
...
```bash
# Set the absolute path to your scideer repo root
export DEER_FLOW_ROOT=/path/to/scideer
...
# .env
OPENAI_API_KEY=sk-...
DEER_FLOW_ROOT=/absolute/path/to/scideer
...
Thread data is stored in `backend/.scideer/threads/`. In Docker deployments, this directory is bind-mounted into the langgraph container.
...
USERDATA_PVC_NAME=scideer-userdata-pvc
SKILLS_PVC_NAME=scideer-skills-pvc
```
...
Application uses [Better Auth](https://www.better-auth.com/) for session management. In production:
