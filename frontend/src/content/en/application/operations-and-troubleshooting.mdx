...
description: This page covers day-to-day operational tasks and solutions to common problems when running Application.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running Application.
...
| File                 | Service                              |
| -------------------- | ------------------------------------ |
| `logs/langgraph.log` | LangGraph / Harness runtime |
...
When you pull a new version of Libra, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.scideer/`.
- Look for memory update errors in `logs/langgraph.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.scideer/`:
...
In Docker deployments, the bind-mounted host path (`$DEER_FLOW_ROOT/backend/.scideer/`) is the source of truth — back up the host path.
